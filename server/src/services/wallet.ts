...
export async function withdrawFromGateway(address: string, amountUsdcStr: string): Promise<string> {
  console.log(`[Gateway Sync] Bypassing on-chain Gateway withdrawal for demo to avoid testnet batching delays...`);
  
  // Simulate network delay
  await new Promise(r => setTimeout(r, 2000));
  
  // We return a mock transaction hash to satisfy the UI, 
  // since the creator already has enough testnet funds in their EOA to test the final payout.
